...
def build_url(file_date):
    try:
        if not file_date:
            raise ValueError("file_date is required")
        ...
        logging.error(f"Failed to build download URL for file_date={file_date}: {e}")
        raise
...
def download_file(file_date):
    try:
        ...
        logging.error(f"Unexpected error while downloading SAM file for {file_date}: {e}")
        raise